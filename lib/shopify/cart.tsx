...
import CartSidebar from "@/lib/shopify/cart"
        ...
        <div ref={ref} className={`flex flex-col ${isMobile ? "" : "md:flex-row"} items-center gap-6 sm:gap-8 md:gap-16 min-h-[calc(100vh-2rem)]`}>
          ...
          <motion.div className="w-full md:w-1/2 flex flex-col items-start text-left px-4 md:px-0 font-sans" initial={{ opacity: 0, x: 50, y: 20 }} animate={isInView ? { opacity: 1, x: 0, y: 0 } : { opacity: 0, x: 50, y: 20 }} transition={{ duration: 0.8, delay: 0.3, type: "spring", stiffness: 100, damping: 20 }}>
            ...
            <div className="flex gap-2 mb-4">
              {variants.map((variant) => (
                <button
                  ...
                  className={`px-4 py-2 border rounded-full text-sm font-medium transition-all ${selectedVariant === variant.id ? 'bg-black text-white' : 'bg-white text-black border-gray-300'}`}
                >
                  {variant.title}
                </button>
              ))}
            ...
            <div className="flex items-center mb-4 gap-2">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="border px-3 py-1 rounded-full">−</button>
              <span className="min-w-[32px] text-center">{quantity}</span>
              <button onClick={() => setQuantity(quantity + 1)} className="border px-3 py-1 rounded-full">+</button>
            </div>
            ...
            <button onClick={goToCheckout} className="bg-[#5a31f4] text-white py-3 px-6 rounded-full text-lg font-semibold w-full max-w-xs flex items-center justify-center gap-1">
              <span className="text-white font-bold">shop</span>
              <span className="bg-white text-[#5a31f4] text-xs px-2 py-0.5 rounded-full font-bold">Pay</span>
            </button>
            ...
            <button
              ...
              className="fixed top-4 right-4 z-50 bg-black text-white px-4 py-2 rounded-full shadow-lg hover:bg-gray-800"
            >
              View Cart
            </button>
